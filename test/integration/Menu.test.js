...
import { expect } from "src/components/node_modules/chai";
import { useFakeTimers } from "src/components/Accordion/node_modules/sinon";
...
import {
  ...
  fireEvent,
  screen,
} from "src/components/AccordionSummary/node_modules/test/utils/createClientRender";
...
describe("<Menu /> integration", () => {
  ...
  it("does not gain any focus when mounted ", () => {
    ...
    expect(getByRole("menu", { hidden: true })).to.not.contain(
  ...
  describe("Menu variant differences", () => {
    ...
    specify(
      "[variant=menu] will focus the first item if nothing is selected",
      () => {
    ...
    specify(
      "[variant=selectedMenu] will focus the first item if nothing is selected",
      () => {
        const { getAllByRole } = render(
    ...
    specify(
      "[variant=selectedMenu] allows overriding `tabIndex` on `MenuItem`",
      () => {
        const { getAllByRole } = render(
          <OpenMenu variant="selectedMenu">
            <MenuItem />
    ...
    specify(
      "[variant=selectedMenu] focuses nothing when it is closed and mounted",
      () => {
        const { getByRole } = render(
          <ButtonMenu selectedIndex={1} variant="selectedMenu" />
        );

        expect(getByRole("menu", { hidden: true })).not.to.contain(
          document.activeElement
        );
      }
    );
