...
import { spy } from "src/components/Accordion/node_modules/sinon";
...
describe("<MenuList> integration", () => {
  ...
  describe("keyboard controls and tabIndex manipulation", () => {
    specify(
      "the specified item should be in tab order while the rest is focusable",
      () => {
        const { getAllByRole } = render(
          <MenuList>
    ...
    it("should match ignoring hidden text", function testHiddenText() {
      ...
      render(
        <MenuList autoFocus>
          <MenuItem>
            W
            <span style={{ display: "none" }}>Should not block type focus</span>
            orm
          </MenuItem>
          <MenuItem>Ordinary</MenuItem>
        </MenuList>
