...
describe("<Select> integration", () => {
  ...
  describe("with Dialog", () => {
    ...
    it("should focus the selected item", () => {
      const { getByTestId, getAllByRole, getByRole, queryByRole } = render(
        <SelectAndDialog />
      );
  ...
  describe("with label", () => {
    ...
    it("does not stays in an active state if an open action did not actually open", () => {
      ...
      expect(container.querySelector('[for="age-simple"]')).to.have.class(
        "focused-label"
      );
      ...
      expect(container.querySelector('[for="age-simple"]')).not.to.have.class(
        "focused-label"
      );
    });
  });
